...
package class WTinyLFUCache<Object: Sendable>: @unchecked Sendable {
  ...
  package func contains(_ key: String) -> Bool {
    guard maximum > 0,
      let hit = data.getNodeAndObject(key, keyHash: stableHash(key))
    else {
      return false
    }
  ...
  private func lookup(_ key: String, keyHash: Int, recordAccess: Bool) -> Object? {
    ...
    if recordAccess, isCacheRunning {
      readBuffer.offer(
        .hit(keyHash: hit.node.keyHash, node: hit.node, generation: hit.node.generation))
      scheduleDrain()
    }
    return hit.object
  }
  ...
  private func scheduleDrain() {
    ...
    guard
      drainScheduled.compareExchange(
        expected: false,
        desired: true,
        ordering: .acquiringAndReleasing
      ).exchanged
...
final class ConcurrentStringMap<Object: Sendable>: @unchecked Sendable {
  ...
  func putOrUpdate(_ key: String, keyHash: Int, _ object: Object, generation: UInt64 = 0)
    -> PutResult<Object>
  {
    let bucket = bucket(forHash: keyHash)
    bucket.lock.lock()
    ...
    let node = Node(
      key: key, object: object, segment: .window, generation: generation, keyHash: keyHash)
...
final class PolicyState<Object: Sendable>: @unchecked Sendable {
  ...
  private func add(_ node: Node<Object>, evictedObjects: inout [Object]) {
    ...
    guard maximum > 0, node.generation == currentGeneration, node.isAlive, node.isLinked == false
    else { return }
...
private func spreadHash(_ hash: Int) -> Int {
