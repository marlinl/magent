  ...
  internal func checkConnect() throws {
    ...
    if version == "HTTP/1.1" {
      guard hostHeaders.count == 1 else {
        throw MagentError.malformedRequest(
          "HTTP CONNECT request must contain exactly one Host header")
      }
    } else {
      guard hostHeaders.count <= 1 else {
    ...
    if let host = hostHeaders.first?.value {
      ...
      guard hostAddress.port == address.port,
        hostAddress.host.caseInsensitiveCompare(address.host) == .orderedSame
      else {
  ...
  internal static func parseAuthority(_ authority: String) throws -> NetworkAddress {
    ...
    if authority.hasPrefix("[") {
      guard let end = authority.firstIndex(of: "]"),
        ...
        authority[authority.index(after: end)] == ":"
      else {
      ...
      guard let colon = authority.lastIndex(of: ":"), authority[..<colon].contains(":") == false
      else {
