...
internal final class Socks5Connection: ChannelInboundHandler, ProxyConnection, @unchecked Sendable {
  ...
  private final class Socks5UDPConnection: ChannelInboundHandler, @unchecked Sendable {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
      ...
      // UDP route futures are created on this relay Channel's EventLoop. Preserve the
      // context-relative read/close semantics while making that ownership explicit to Swift 6.
...
extension Socks5Connection {
  ...
  func upstream(context: ChannelHandlerContext, data: NIOAny) {
    ...
    case .request:
      do {
        ...
        isReadingInitialRequest = false
        let responseFuture =
          respondProxyChannelOnce(Self.reply(Self.replyCode(for: error)))
  ...
  private func installTCPBind(_ address: NetworkAddress) throws {
    ...
    if let wire = try core.routeTCPWire(address) {
      ...
      core.createTCPClientChannel(
        ...
        let responseFuture =
          self.respondProxyChannelOnce(Self.reply(Self.replyCode(for: error)))
  ...
  private func installUDPAssociate() throws {
    ...
    _ = core.createUDPClientChannel(
      ...
      guard
        let responseFuture = self.respondProxyChannelOnce(
  ...
  private func startTunnel(_ channel: Channel?) -> EventLoopFuture<Void> {
    ...
    guard
      let responseFuture = respondProxyChannelOnce(
        Self.reply(.succeeded, boundAddress: boundAddress))
    else {
      channel.close(promise: nil)
      return proxyChannel.eventLoop.makeSucceededFuture(())
  ...
  private func respondProxyChannelOnce(_ data: Data) -> EventLoopFuture<Void>? {
    guard
      hasRespondedHandshake.compareExchange(
        expected: false,
        desired: true,
...
extension Socks5Connection {
  ...
  private static func paserSocks5(_ input: ByteBuffer) throws -> (
    command: Socks5Command, address: NetworkAddress
  ) {
    let data = Data(input.readableBytesView)
    guard data.count >= 4 else {
    ...
    guard let (address, _) = try parseAddress(data, offset: 3, zeroPort: command == .udpAssociate)
    else {
  ...
  private static func parseAddress(_ data: Data, offset: Int, zeroPort: Bool) throws -> (
    NetworkAddress, Int
  )? {
