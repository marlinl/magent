  ...
  internal init(cipher: ProxyCipher, key: Data, nonce: Data? = nil) throws {
    guard key.count == cipher.keySize else {
      throw MagentError.cryptoError(
        type: "invalidKeyLength", expected: cipher.keySize, actual: key.count)
    }
    if let nonce, nonce.count != cipher.nonceSize {
      throw MagentError.cryptoError(
        type: "invalidIVLength", expected: cipher.nonceSize, actual: nonce.count)
    }
    self.cipher = cipher
    self.key = key
...
private func littleEndianWords(_ data: Data) -> [UInt32] {
  precondition(
    data.count.isMultiple(of: 4), "Little-endian word input must be 4-byte aligned in length")
  var words: [UInt32] = []
