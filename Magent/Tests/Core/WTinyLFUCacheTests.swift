...
final class WTinyLFUCacheTests: XCTestCase {
  ...
  func testAfterWriteExpiresEntryAndEvictsOnce() {
    ...
    let cache = WTinyLFUCache<String>(capacity: 4, expiration: .afterWrite(.milliseconds(50))) {
      _ in
      evictions.increment()
    }
    defer { shutdown(cache) }
...
final class WTinyLFUInternalStructureTests: XCTestCase {
  ...
  func testPolicyClearIgnoresStaleReadEventsFromPreviousGeneration() throws {
    ...
    policy.applyReads(
      Array(repeating: ReadEvent<Int>.miss(keyHash: staleHash, generation: 0), count: 100))
  ...
  func testPolicyAdmitsHighFrequencyCandidateOverLowerFrequencyVictim() throws {
    ...
    policy.applyReads(
      Array(
        repeating: ReadEvent<Int>.hit(keyHash: candidate.keyHash, node: candidate, generation: 0),
        count: 5)
    )
  ...
  func testPolicyRejectsLowFrequencyCandidateAgainstHotVictim() throws {
    ...
    policy.applyReads(
      Array(
        repeating: ReadEvent<Int>.hit(keyHash: victim.keyHash, node: victim, generation: 0),
        count: 5)
    )
