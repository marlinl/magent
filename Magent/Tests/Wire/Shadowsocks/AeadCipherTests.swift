...
final class AeadCipherTests: XCTestCase {
  ...
  func testAeadInvalidKeyLengthThrows() {
    for cipher in ProxyCipher.allCases {
      XCTAssertThrowsError(try AeadCipher(cipher: cipher, key: Data(repeating: 0, count: 1))) {
        error in
        guard let magentError = error as? MagentError,
          case .cryptoError(let description) = magentError
        else {
  ...
  private func assertAeadRoundTrip(
    cipher: ProxyCipher, plaintext: Data, file: StaticString = #filePath,
    line: UInt = #line
  ) throws {
    let key = makeKey(cipher)
    let enc = try AeadCipher(cipher: cipher, key: key)
