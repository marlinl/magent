...
final class HttpForwardConnectionTests: XCTestCase {
  ...
  func testHTTPForwardProxyWritesShadowsocksHandshakeBeforeRequest() throws {
    ...
    let handshakeLength =
      cipher.saltSize + 2 + cipher.tagSize + shadowsocksAddressLength + cipher.tagSize
    ...
    XCTAssertEqual(
      try encryptedStreamPromise.futureResult.wait().count, handshakeLength + requestLength)
  ...
  func testHTTPForwardReturnsSingleBadGatewayWhenDirectConnectFails() throws {
    ...
    let clientClosed = expectation(
      description: "HTTP forward closes after the bad gateway response")
    try client.pipeline.addHandler(
      TestDataCollector(
        expectedByteCount: HttpForwardConnection.badGateway.count, promise: responsePromise)
    ).wait()
  ...
  func testHTTPForwardWaitsForProxyWriteBeforeReadingNextTargetBatch() throws {
    ...
    let delayedWrites = DelayedTestWrites(
      eventLoop: eventLoop, firstWritePromise: firstProxyWritePromise)
    let responseHead = Data(
      "HTTP/1.1 200 OK\r\nContent-Length: 4\r\nConnection: close\r\n\r\n".utf8)
  ...
  func testHTTPForwardNumericTargetsPreserveIPAddressType() throws {
    ...
    for request in [absoluteIPv6, hostIPv6] {
      guard
        case .ipv6(let bytes, let port) = try HttpForwardConnection.buildRequest(
          head: request,
          body: Data()
        ).target
      else {
