---
id: SOCKS5-C21
title: "restart 关闭旧 runtime，与保留旧会话及优雅排空的 spec 不同"
...
# SOCKS5-C21：restart 关闭旧 runtime，与保留旧会话及优雅排空的 spec 不同
...
- 规格：[SOCKS5_PROXY_SPEC.md](../SOCKS5_PROXY_SPEC.md)，§19.5–19.7；C05–C07/L08。
...
restart 先创建新 Core，随后完成旧 shutdownPromise 并关闭旧 listener/accepted connections；close 直接结束运行周期和 group。没有保留旧会话的路由 reload 或 shutdown grace 阶段。

## 影响与复现边界

配置校验失败不关闭旧 runtime 已有保障；但合法 restart 会断开旧 TCP/UDP 会话，不能满足 spec 的仅新会话用新配置。现有生命周期文档明确描述了关闭旧会话。
...
先选择维持 terminal close/restart 合同，或新增明确的 reload/drain 产品操作；不要改动既有名称却隐藏行为变化。

### 修复方案

1. 先利用现有每运行周期 Core/关闭 promise 的实例隔离，不默认引入通用 immutable snapshot 框架。
2. 若支持路由 reload，已有连接继续持有旧 Core，新 accept 从明确同步的 runtime owner 获得新 Core。
3. 监听参数变更使用受控 restart；不伪装为普通规则 reload。
4. 优雅 shutdown 停止新 accept 和新 flow，排空期结束后完成每个 runtime 的关闭信号，再关闭 service-owned group。

依赖：C05、C06、C20、C22。

设计约束/决策：当前 AGENTS 明确 `restart` 创建新 Core 并通知旧 runtime 关闭，`close` 终止 group。此项必须作为产品合同决策，不能为 spec 测试偷偷替换。
...
```swift
// Magent 现有文件中的生命周期设计伪代码；不是当前 API。
let nextCore = try buildValidatedCore(newConfig)
if listenerSettingsChanged {
  return .requiresRestart
}
// 原子更新“未来 accept 使用的 Core”；老连接保存其原 Core。
installCoreForNewConnections(nextCore)
// drain：stopAccepting -> forbidNewFlows -> deadline -> signalRuntimes -> shutdownGroup
// 永不复用已经完成的 shutdownPromise。
```
...
- [ ] 旧 TCP 流和 UDP 关联的新目标仍用旧 Core；新会话用新 Core。
- [ ] 无效新配置不影响旧连接；监听变更给出明确 restart 结果。
- [ ] 排空到期后资源/预算归零，任务结束与 owner 释放顺序可证明。
