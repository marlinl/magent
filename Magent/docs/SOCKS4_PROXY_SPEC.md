...
**版本 0.1.0 · 2026-09-21 · 设计与实现验收基线**

本单文件包含 15 份 Markdown 正文，以及配置、JSON Schema、解析向量和测试工具的完整文本。单文件内链接已改为锚点；分文件 ZIP 保留原始目录结构。

这是规格与测试辅助资料，不是已实现的代理服务。默认端口、路由和资源阈值是本稿设计；真实节点参数需要按工程配置填写。
...
**版本：0.1.0 · 状态：供实现与评审的设计基线 · 日期：2026-09-21**
...
资料核对日期：**2026-09-21**。协议事实优先采用原始协议、RFC 和项目官方文档；代码仓库默认分支会变化，真正发布实现时应锁定依赖版本和参考 commit，不能把本文的核对日期当作代码版本号。
