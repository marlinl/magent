...
- Contract：包内 API、调用方契约和内部模块契约
- Core Logic：核心读写链路、W-TinyLFU 策略和容量收敛
- Corners：边界场景、测试、benchmark、观测和降级

本文描述当前实现及已知限制；第 5 节保留历史 benchmark 归档，不代表当前版本已经重新测得相同结果。
...
| 入口 | 定位 |
|---|---|
| `WTinyLFUCache<Object: Sendable>` | Swift package 内可见的通用泛型缓存 |
| `MagentCache<Value: Sendable>` | Magent 模块内部的泛型入口，当前 Core 使用 `MagentCache<Decision>` |

`MagentCache` 继承 `WTinyLFUCache` 并透传初始化参数，不重新实现策略；value 类型由调用方指定。两者都不是 Magent 库的 public API。
...
| API 简洁 | 初始化提供 `capacity`、`expiration`、`onEvict`，不公开内部策略参数 |
| NIO 友好 | 包内 API 同步，maintenance 使用实例串行 `DispatchQueue`，共享系统 executor |
| 并发安全 | map、buffer、policy maintenance 分层处理并发读写 |
| 可观测 | 提供 `estimatedSize()` 和删除通知 `onEvict`；命中率等由调用方统计 |
| 可测试 | XCTest 覆盖 API、内部结构操作和并发容量；没有完整结构校验 API |
...
- 定义业务 key 格式。
- 业务层的 host normalize、trim、lowercase 或 key 校验；key 相等遵循 Swift String 规范等价语义。
- 定义业务 value schema。
- 推断业务版本、tenant 或 namespace；TTL 由调用方通过 `expiration` 显式配置。
...
以下为 `MagentCache.swift` 的签名摘要，不是独立的 `Cache` 协议：
...
- loader 是调用点传入的同步闭包，cache 不保存 loader，不返回 `EventLoopFuture`，也不接收外部 `EventLoop`。
- `get` 返回 `Object?`，缺少有效 entry 时返回外层 `nil`。`getOrLoad` 返回 `Object`；空结果应由业务 value 类型表达。
- shutdown 后 `getOrLoad` 直接 bypass 到 loader，不写回缓存。其他操作的 shutdown 语义见 §4.8。
...
- miss 时由当前调用线程同步执行。
- loader 完成后先二次查询或尝试写入，再同步返回结果；是否长期保留由 W-TinyLFU admission 决定。
- 同 key 并发 miss 可能重复调用 loader。
- loader 返回后，cache 会二次 lookup；若此时命中有效 entry，则返回已有对象。二次 lookup 与后续 put 不是一个原子操作，不承诺并发 loader 只产生或返回同一个对象。
- 慢 loader、异步 I/O、失败重试、single-flight 都应在 cache 外层处理。

## 2.3 Key / Object 契约

key 相等遵循普通 Swift `String` 语义。Unicode 规范等价的不同编码共用一个 entry，例如 `"\u{00E9}"` 与 `"e\u{0301}"`。分桶、字典和频率草图共用 `stableHash`：ASCII 直接计算 FNV-1a，非 ASCII 先转换为 NFC 再计算；存储的 key 和 loader 收到的原字符串不变。

缓存不做大小写折叠、空白裁剪或 Unicode 兼容归一化。以下值互不相同：
...
`Object` 由调用方定义并必须满足 `Sendable`。若它是 class，cache 保存引用，不 copy，也不保护对象内部可变状态；调用方需要保证对象不可变或线程安全。

## 2.4 过期与删除通知契约

| 策略 | 当前行为 |
|---|---|
| `.never` | 不进行时间过期检查，也不启动过期 timer |
| `.afterWrite(duration)` | 从创建或最近一次更新写入开始计算 TTL；读取不续期 |
| `.afterRead(duration)` | 新节点从创建时开始计算 TTL；运行中的成功 lookup 尝试续期，未过期节点的 `put` 和 `contains` 不续期 |

TTL 以单调时钟的毫秒计时，配置转换成毫秒后必须大于 0。`get`、`getOrLoad` 的 lookup 和 `contains` 都检查过期；运行中校验到过期节点时，尝试通过时间戳 CAS 和节点身份校验删除。`estimatedSize()` 不主动清理过期项。

启用 TTL 时，maintenance queue 每 10 秒抽样检查约 5% 的 entry。抽样当前使用遍历 map 的 reservoir sampling，因此抽样数量为 5% 不代表只遍历 5%，也不保证到期即回收。

对于两种 TTL 策略，`put` 在写入临界区内先检查已有节点：未过期时原地更新，已过期时先认领并删除旧节点，再创建带有当前时间戳的新节点。`.afterRead` 的未过期更新保留原读取期限，`.afterWrite` 的未过期更新刷新写入时间。过期后重写从新节点的创建时间重新计算 TTL。

写入和读取校验共用过期删除逻辑：先对观察到的时间戳执行 CAS，再按节点身份从 map 删除。并发读取若已续期，写入路径在 CAS 失败后重新检查；容量淘汰若先删除旧节点，则不重复通知。旧节点的延迟清理不能删除同 key 的替换节点。旧节点 remove 事件先于新节点 add 事件提交，删除通知在写入锁释放后执行。

`onEvict` 在成功从 map 移除节点后收到该节点的 object，涵盖容量淘汰、准入拒绝、TTL 删除、`invalidate` 和 `removeAll`。同一节点只有取得删除所有权的路径负责通知；原地更新 value 不通知旧值，shutdown 本身也不等于清空或逐项通知。

通知发生在内部锁释放后，允许回调重新进入 cache；它可能运行在调用线程或 maintenance queue，调用方不得假设固定线程或串行回调，也不应阻塞 maintenance。

## 2.5 观测契约

当前没有 `recordStats()`、`statsSnapshot()` 或 `WTinyLFUStatsSnapshot`。`estimatedSize()` 提供 map 条目数估算；`onEvict` 提供删除对象，但不携带删除原因，不能据此区分 admission rejection、容量淘汰和主动失效。命中率、loader 调用数等由调用方或 benchmark 自行统计。
...
WTinyLFUCache<Object>
  ...
  PolicyState
    ...
    Main Protected
    FrequencySketch
  serial DispatchQueue / optional DispatchSourceTimer
```
...
- write buffer 有界地记录 add/remove/clear 写事件；满时由写入线程同步推进 maintenance，写事件不因满而丢弃。
- policy 只在 maintenance drain 时维护 window、probation、protected 和 sketch。
- 每个实例的串行 `DispatchQueue` 负责异步 maintenance 和过期扫描，不创建专用 `EventLoopGroup` 或线程；业务 loader 在调用线程执行。写入压力下的同步 drain 由 maintenance 锁与队列上的 drain 串行化。
...
map hit
  -> 校验过期；失效则返回 nil
  -> 运行中的 afterRead 尝试续期
  -> 运行中 offer read hit / schedule drain
  -> 返回 object

map miss
  -> 运行中 offer read miss / schedule drain
  -> 返回 nil
```

读事件用于更新频率草图和访问顺序，不改变当前返回值。`contains` 只查存在性和过期，不记录访问事件，也不续期。
...
```text
capacity == 0 或已经 shutdown
  -> no-op

key exists and unexpired
  -> 更新 object；afterWrite 刷新写入时间，afterRead 保留读取期限
  -> offer read hit
  -> schedule drain

key exists and expired
  -> CAS 认领并按节点身份删除旧节点；并发续期则重新检查
  -> 成功删除时收集旧 object 并提交 write remove
  -> 继续新节点写入流程
...
写入路径在释放 writeLock 后通知本次实际删除的对象。`invalidate(key)` 从 map 删除 key，并提交 remove event。key 不存在时不报错。
...
当 `estimatedSize()` 达到该 transient budget 时，写入路径会 drain write buffer，直到 map 回到 `capacity` 内或没有 pending write event。
...
包内 API 的边界是同步、窄配置、无业务语义：

| 约束 | 说明 | 覆盖测试 |
|---|---|---|
| miss 返回 `nil` | `get` 不触发 loader，也不创建 entry | `testGetReturnsNilForMissingKey` |
| `put` 后可读回 | 默认不过期配置下，同 key 应命中写入值 | `testPutThenGetReturnsStoredObject` |
...
| `MagentCache` 只做包装 | 泛型入口复用同一 loading 语义，Core 使用 `Decision` | `testGetOrLoadCachesDecision` |
| Unicode 规范等价 | 不同编码共用 entry；读、更新、加载、删除一致 | `testCanonicallyEquivalentKeysShareEntry` |
| 非等价 key 分离 | 保留大小写、空白和兼容字符区别 | `testDistinctKeysPreserveCaseWhitespaceAndCompatibilityCharacters` |
| TTL 到期 | afterWrite 到期删除并通知一次；afterRead 命中续期 | `testAfterWriteExpiresEntryAndEvictsOnce`, `testAfterReadRenewsEntryUntilLastReadExpires` |
| 删除回调可重入 | 回调在锁外运行，可重新调用 cache | `testOnEvictCanReenterCache` |
| 过期后重写 | 新节点立即可读，旧对象只通知一次，回调可重入 | `testExpiredPutCreatesFreshEntryAndEvictsOldValueOnce` |
| 未过期更新的 TTL | afterRead 更新和 contains 不续期；afterWrite 更新续期 | `testAfterReadPutAndContainsDoNotRenewUnexpiredEntry`, `testAfterWritePutRenewsUnexpiredEntry` |
| 续期与并发写入 | 已续期节点在并发读写和过期校验下保持有效 | `testAfterReadRenewalSurvivesConcurrentWritesAndExpirationChecks` |
| 过期删除竞争 | 读校验与重写竞争时只通知旧节点，新节点不被旧事件删除 | `testExpiredPutRacingReadsEvictsOnlyOldNode`, `testPolicyStaleExpirationEventsPreserveReplacement` |

包内 API 不暴露 window/main/protected、sketch、buffer 或 EventLoop 参数。TTL 测试使用真实单调时钟；并发测试覆盖代表性交错，不等于穷尽所有调度顺序。
...
| 并发确定性 loader | loader 对同 key 总返回同一规范值时，返回值保持正确；不测试不同 loader 结果的唯一性 | `testConcurrentGetOrLoadReturnsDeterministicValue` |
...
| 重负载混合操作稳定 | 50 万次混合操作后 size 回到 capacity 以内 | `testHeavyConcurrentMixStaysBounded` |
| 长跑不死锁不泄漏 size | 多 worker 长跑结束后仍可读写，size 有界 | `testLongRunningMixedOpsStayStableAndBounded` |
...
| 混合并发收敛 | 并发 mutation、removeAll、loading 混合后容量收敛 | `testHeavyConcurrentMixStaysBounded` |
| policy 默认行为可观测 | benchmarker 自行记录 peak/final size 与 hit rate | `default-policy-validation` benchmark phase |
...
这是实现采用的 transient budget。部分测试和当前 benchmark 使用较宽松的 `2 * capacity` 上界，不能将其误认为所有容量下的精确触发阈值。
...
| scan 保留热点 | scan 写入下记录热点命中率、生存率及最终 size | `scan-hot-retention` benchmark phase |

策略相关文档必须保留 `candidateFrequency > victimFrequency` 的严格比较；不能改成 `>=`。

## 4.6 内部结构边界

内部结构不是 public API。现有定向测试覆盖以下结构操作和约束：
...
当前没有 `debugValidate()` 或 `testDebugValidatePassesAfterMixedMutations`。上表中的局部结构测试和并发压测，不能代替完整 drain 后的 map/deque 双向一致性检查，也不能证明所有并发交错下都没有数据竞争。

## 4.7 观测与验证边界

benchmarker 自行统计 QPS、latency、hit rate、loader 调用数和 estimated size。当前 cache 没有 stats 开关、snapshot API 或对应的 stats 测试，也没有 admission/rejection 的精确计数接口。历史 benchmark 中出现的相关计数只属于当时版本。

已有测试分别证明其断言覆盖的行为；容量最终收敛不等于所有 pending maintenance 都已 drain，普通压测通过也不等于已执行 Thread Sanitizer 或完整结构验证。

## 4.8 ARC / 生命周期边界

cache 持有实例串行 `DispatchQueue` 和可选的过期 timer；异步 drain 和 timer handler 使用弱引用访问 cache。当前没有显式 `deinit` 实现，也不拥有 NIO `EventLoopGroup`。

需要停止后续 mutation 并等待当前队列上的 maintenance 工作结束时，调用：

```swift
cache.shutdownGracefully { error in
    // completion callback on the instance maintenance queue
}
```

shutdown 将状态标记为关闭，取消过期 timer，等待已进入 write 临界区的 mutation 离开，再将缓冲清理和完成回调排到 maintenance queue 尾部。缓冲中的剩余事件会被丢弃，不保证全部应用到 policy。重复调用的每个 callback 都会排队执行，当前实现回调 `nil`。

shutdown 后 `put`、`invalidate`、`removeAll` 均为 no-op；`getOrLoad` 总是调用 loader；`get` 和 `contains` 仍可读取尚未过期的已有 entry，但不会续期或删除到期节点。shutdown 不清空 map，也不为剩余对象逐项调用 `onEvict`。需要清空并通知时，应由调用方停止业务访问后，先 `removeAll()` 再 shutdown。

回调在 maintenance queue 上执行，不要在该队列或 `onEvict` 回调中同步等待同一 cache 的 shutdown 完成。

| 约束 | 说明 | 覆盖测试 |
|---|---|---|
| shutdown 后禁止 mutation | 已有对象仍可读取，新写入和删除不生效 | `testShutdownMakesAllMutationsNoOps` |
| shutdown 写入屏障 | 完成后已发起的并发写入也不能改变 size | `testShutdownCallbackFormsMutationBarrier` |
| 重复 shutdown 回调 | 每次调用均收到完成通知 | `testRepeatedShutdownCompletesEveryCallback` |
| removeAll 后释放对象 | 清空、shutdown 并释放 cache 后对象无 ARC 保留环 | `testRemoveAllAndShutdownReleaseCachedObjects` |
| shutdown 后不写回 | `getOrLoad` 只调用 loader | `testShutdownGetOrLoadBypassesCacheAndUsesLoader` |
...
本文只列出现存 API 和测试，不将已删除的 stats、结构校验或 EventLoopGroup 生命周期描述为当前能力。Unicode key 语义、TTL 过期后重写、未过期更新和删除通知由 §4.1 列出的回归测试覆盖；完整结构校验与所有并发交错的证明仍不在现有测试覆盖范围内。

---

# 5. Benchmarker

`WTinyLFUCacheBenchmark` 是当前 benchmarker executable target。§5.1–5.3 为历史设备和运行结果，保留原始数值；其中旧 stats 字段不表示当前 executable 仍输出这些字段，也不代表当前版本的性能。
...
## 5.2 2026-07-02 quick-smoke 历史数据
...
```text
package path:
  get / contains / getOrLoad / put / invalidate / removeAll
...
包内 API 提供 `capacity`、`expiration`、`onEvict`、同步 loader、基础 CRUD、`estimatedSize()` 和 shutdown。业务 key、业务 value、版本隔离、异步加载、错误语义和指标上报都留在调用方；当前不提供 stats snapshot 或完整内部结构校验 API。
