  ...
  func sync() {
    ...
    localExecutor.submit(
      ...
      operation: {
        ...
        let rulesByIdentity = Dictionary(
          uniqueKeysWithValues: existingRules.map { existingRule in
            (
              RuleIdentity(
        ...
        let proxyRules = downloadedRules.map { downloadedRule in
          ...
          if let existingRule = rulesByIdentity[identity] {
            ...
            return (rule: existingRule, isNew: false)
          }
        ...
        // 4. 已有规则已原位更新，仅插入新增规则，最后统一保存。
        for proxyRule in proxyRules where proxyRule.isNew {
          modelContext.insert(proxyRule.rule)
        }
