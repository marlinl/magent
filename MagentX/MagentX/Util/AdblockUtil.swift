...
nonisolated struct AdblockRule: Hashable, Sendable {
  /// 规则的语义类别。
  ...
  /// Cosmetic 规则分隔符的语义。
  nonisolated enum CosmeticKind: String, Hashable, Sendable {
    /// 隐藏元素。
    case hide = "##"
  ...
  /// Cosmetic 规则的域名条件。
  nonisolated struct Domain: Hashable, Sendable {
    /// 不带排除前缀的域名文本。
    let value: String
...
nonisolated enum AdblockRuleParser {
  /// 解析多行规则，忽略空行、注释和元数据行。
  ///
