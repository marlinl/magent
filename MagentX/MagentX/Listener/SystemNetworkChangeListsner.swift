...
final class SystemNetworkChangeListsner: ObservableObject {
  ...
  /// 接收系统回调，并将最近成功启动的代理配置重新写入系统网络服务。
...
private final class SystemNetworkProxyPreferences {
  ...
  private func updateProxyProtocols(
    ...
    where Self.shouldUpdate(service: service, activeServiceIDs: activeServiceIDs) {
      guard let proxyProtocol = SCNetworkServiceCopyProtocol(service, kSCNetworkProtocolTypeProxies)
      else {
        continue
      }
      ...
      guard SCNetworkProtocolSetConfiguration(proxyProtocol, proxyConfiguration as CFDictionary)
      else {
        throw MagentXError.systemNetworkProxyConfigurationFailed(Self.systemConfigurationError())
      }
