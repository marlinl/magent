...
nonisolated struct CurrentSelection: Sendable {
  ...
  static func load(userDefaults: UserDefaults = .standard) -> CurrentSelection {
    CurrentSelection(
      state: BackgroundServiceState(rawValue: userDefaults.string(forKey: stateStorageKey) ?? "")
        ?? .stop,
      mode: SystemProxyMode(rawValue: userDefaults.string(forKey: "currentSelection.mode") ?? "")
        ?? .pac
    )
  }
